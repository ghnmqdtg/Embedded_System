...
typedef struct paddle_right paddle_right;
...
int main(void)
{
  ...
  ball.x = center_x;
  ball.y = center_y;
  // BSP_LCD_DisplayStringAtLine(0, (uint8_t *)"Hello");  // For testing the LCD
  ...
  while (1)
  {
    ...
    if(mode == 0) {
        ...
        tk_current = HAL_GetTick();
        if(tk_current - tk_record > 150) {
          tk_record = tk_current;
      ...
      if(tk_current - tk_record > 50) {
        ...
        if(TS_State.touchDetected) {
          for(uint8_t i = 0; i < 2; i++) {
            ...
            // Display the info of touched coordinates(for debugging)
            sprintf(str, "x[%d]: %03d, y[%d]: %03d", i, TS_State.touchX[i], i, TS_State.touchY[i]);
            BSP_LCD_DisplayStringAtLine(i+1, (uint8_t*) str);
            // sprintf(str, "paddle_L.y: %03d", paddle_L.y);
            // BSP_LCD_DisplayStringAtLine(3, (uint8_t*) str);
            // sprintf(str, "paddle_R.y: %03d", paddle_R.y);
      ...
      ball.x = ball.x + distance_x * speed;
      ball.y = ball.y + distance_y * speed;
      ...
      // the ball meets paddles
      if((ball.x == paddle_L.x + 10) || (ball.x == paddle_R.x - 10)) {
        if((ball.y > paddle_L.y - paddle_height / 2) && (ball.y < paddle_L.y + paddle_height)) {
          distance_x = -1 * distance_x;
        }
        else if((ball.y > paddle_R.y - paddle_height / 2) && (ball.y < paddle_R.y + paddle_height)) {
          distance_x = -1 * distance_x;
        }
      }
      // out of bounds
      else if(ball.x <= 0 || ball.x >= screen_width) {
          ball.x = center_x;
          ball.y = center_y;
      }
