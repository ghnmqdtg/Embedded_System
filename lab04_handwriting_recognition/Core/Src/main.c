...
int main(void)
{
  ...
  // Define the in_data and out_data
  // AI_MNETWORK_IN_1_SIZE_BYTES is defined in "app_x-cube-ai.h"
  static ai_float in_data[784];
  static ai_float out_data[10];
  ...
  while (1)
  {
    ...
    if (tick_current - tk_record > 10) {
      tk_record = tick_current;
      BSP_TS_GetState(&TS_State);
      if (TS_State.touchDetected) {
        // Check the position of the touch
        if (TS_State.touchY[0] >= 24 + 5 && TS_State.touchY[0] <= 248 - 5) {
          // Right: store the touched position and draw it on the screen
          if (TS_State.touchX[0] >= 8 + 5 && TS_State.touchX[0] <= 232 - 5 && mode == 0) {
            // Every square is 8 * 8
            ...
            // For Debugging
            // sprintf(str, "Input = %f at %d\n", in_data[position], position);
            // BSP_LCD_DisplayStringAtLine(0, (uint8_t *)str);

            TS_State.touchX[0] = TS_State.touchY[0] = 0;
          }
          // Left: switch the mode by clicking
          else if (TS_State.touchX[0] >= 248 + 5 && TS_State.touchX[0] <= 472 - 5) {
            switch (mode) {
              // Predict the number
              case 0:
                // For Debugging
                for(int i = 1; i <= 784; i++) {
                  printf("%1.0f ", in_data[i - 1]);
                  if(i % 28 == 0) {
                    printf("\r\n");
                  }
                  if(in_data[i - 1] > 0) {
                ...
                if(checked) {
                  ...
                  int index = -1;
                  // storing the largest number in out_data to output
                  for (int i = 0; i < 10; i++) {
                    printf("%1.5f ", out_data[i]);
                    if (output < out_data[i]) {
                      output = out_data[i];
                      index = i;
                    }
                  }
                  ...
                  sprintf(str, "%d", index);
                  BSP_LCD_DisplayStringAt(340, 110, (uint8_t *)str, LEFT_MODE);                  
